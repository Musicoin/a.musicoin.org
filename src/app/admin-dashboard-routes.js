    ...
    constructor(musicoinApi, jsonAPI, addressResolver, maxImageWidth, mediaProvider, // TODO
        ...
        router.post('/releases/link', function (req, res) {
            return Release.find({ artist: { $exists: false } }).limit(100).exec()
                .then(releases => {
                return bluebird_1.Promise.all(releases.map(r => {
        ...
        router.post('/elements/release-count', function (req, res) {
            ...
            return bluebird_1.Promise.join(a, d, (all, day) => {
        ...
        router.post('/elements/artist-count', function (req, res) {
            ...
            return bluebird_1.Promise.join(a, v, (artistCount, verifiedCount) => {
                doRender(req, res, 'admin/count.ejs', {
                    count: artistCount,
                    type: "Total Artists",
        ...
        router.post('/elements/play-count', function (req, res) {
            return ReleaseStats.aggregate({ $match: { duration: "all" } }, { $group: { _id: "all", plays: { $sum: "$playCount" } } })
                .then(results => {
        ...
        router.post('/elements/tip-count', function (req, res) {
            const releaseTips = ReleaseStats.aggregate({ $match: { duration: "all" } }, { $group: { _id: "all", tips: { $sum: "$tipCount" } } });
            const userTips = UserStats.aggregate({ $match: { duration: "all" } }, { $group: { _id: "all", tips: { $sum: "$tipCount" } } });
            return bluebird_1.Promise.join(releaseTips, userTips, (releaseResults, userResults) => {
        ...
        router.post('/elements/users', function (req, res) {
            ...
            jsonAPI.getAllUsers(req.body.search, invitedByIds, req.body.verified, req.body.artist, start, length)
                ...
                const balanceMap = {};
                const ivb = req.body.invitedby && invitedByIds.length == 1 ? User.findById(invitedByIds[0]).exec() : bluebird_1.Promise.resolve(null);
                bluebird_1.Promise.join(musicoinApi.getAccountBalances(addresses), ivb, (balances, invitedBy) => {
        ...
        router.post('/elements/account-balances', function (req, res) {
            ...
            bluebird_1.Promise.join(b, o, (mcBalance, balances) => {
                const output = [];
        ...
        router.post('/api-clients/delete', (req, res) => {
            APIClient.findById(req.body.id).exec()
                .then(client => {
                if (!client)
                    throw Error("Could not find requested client");
