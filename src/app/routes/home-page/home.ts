...
router.get('/main', functions.isLoggedIn, function (req, res) {
  ...
  Promise.join(rs, fa, b, h, tpw, ttw, function (releases, artists, balance, hero, topPlayed, topTipped) {
    return functions.doRender(req, res, "index-new.ejs", {
...
router.get('/feed', functions.isLoggedIn, function (req, res) {
  ...
  Promise.join(m, h, tpw, ttw, r, function (messages, hero, topPlayed, topTipped, recentlyPlayed) {
    if (messages.length > 0) {
      ...
      return functions.doRender(req, res, "feed.ejs", {
...
router.get('/thread-page', function (req, res) {
  ...
  handleMessagePost(req).then(() => jsonAPI.getThreadMessages(req.query.thread, limit))
    .then(messages => {
      return functions.doRender(req, res, "thread.ejs", {
        messages: messages,
    ...
    .catch(err => {
      ...
      return functions.doRender(req, res, "thread.ejs", { messages: [] });
    })
});
...
router.post('/thread-view', function (req, res) {
  ...
  handleMessagePost(req).then(() => jsonAPI.getThreadMessages(req.query.thread, limit))
    .then(messages => {
      return functions.doRender(req, res, "thread-view.ejs", {
        messages: messages,
        threadId: req.query.thread,
        threadUrl: `${config.serverEndpoint}/thread-page/?thread=${req.query.thread}`,
        showTrack: showTrack,
    ...
    .catch(err => {
      console.log("Failed to load thread messages: " + err);
      return functions.doRender(req, res, "thread-view.ejs", { messages: [] });
    })
});
...
router.post('/elements/thread', function (req, res) {
  ...
  handleMessagePost(req).then(() => jsonAPI.getThreadMessages(threadId, limit))
    .then(messages => {
      return functions.doRender(req, res, "partials/track-messages.ejs", { messages: messages, showTrack: showTrack });
    })
    .catch(err => {
      console.log("Failed to load track messages: " + err);
      return functions.doRender(req, res, "partials/track-messages.ejs", { messages: [] });
    })
});
...
router.post('/elements/user-messages', function (req, res) {
  ...
  handleMessagePost(req).then(() => jsonAPI.getUserMessages(profileAddress, limit))
    .then(messages => {
      return functions.doRender(req, res, "partials/track-messages.ejs", { messages: messages, showTrack: showTrack, noContentMessage: noContentMessage });
    })
    .catch(err => {
      console.log("Failed to load track messages: " + err);
      return functions.doRender(req, res, "partials/track-messages.ejs", { messages: [] });
...
router.post('/elements/feed', function (req, res) {
  ...
  handleMessagePost(req).then(() => jsonAPI.getFeedMessages(req.user._id, limit, messageTypes))
    .then(messages => {
      return functions.doRender(req, res, "partials/track-messages.ejs", {
        messages: messages,
        showTrack: showTrack,
...
router.get('/tx/history/:address', functions.isLoggedIn, function (req, res) {
  ...
  Promise.join(
    ...
    function (history, name) {
      ...
      //  console.log(e);
      // });
      return functions.doRender(req, res, 'history.ejs', {
...
// router.get('/landing',  (req, res) => functions.doRender(req, res, 'landing.ejs', {}));
router.get('/welcome', functions.redirectIfLoggedIn('/loginRedirect'), (req, res) => {
  if (req.user) {
  ...
  return functions.doRender(req, res, 'landing.ejs', {
    message: message,
    code: req.session.inviteCode
  });
});
router.get('/welcome-musician', functions.redirectIfLoggedIn('/loginRedirect'), (req, res) => {
  ...
  const message = req.flash('loginMessage');
  return functions.doRender(req, res, 'landing_musicians.ejs', {
    message: message,
    code: req.session.inviteCode
  });
});
...
router.get('/terms', (req, res) => functions.doRender(req, res, 'terms.ejs', {}));
router.get('/error', (req, res) => functions.doRender(req, res, 'error.ejs', {}));
...
function handleBrowseRequest(req, res, _search, genre) {
  ...
  Promise.join(rs, as, function (releases, artists) {
    return this.functions.doRender(req, res, "browse.ejs", {
      searchTerm: search,
      genreFilter: genre,
      releases: releases,
