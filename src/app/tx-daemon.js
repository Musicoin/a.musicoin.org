...
class PendingTxDaemon {
    constructor(newProfileCallback, releaseCallback) {
        this.newProfileCallback = newProfileCallback;
        this.releaseCallback = releaseCallback;
    }
    start(musicoinApi, intervalMs) {
    ...
    checkForPendingProfilesUpdates(musicoinApi) {
        User.find({ updatePending: true }, function (err, results) {
            if (err)
                console.log(`Failed to check for pending releases: ${err}`);
            else {
                if (results.length == 0)
                    return;
    ...
    updatePendingProfileStatus(musicoinApi, p) {
    ...
    checkForPendingPPPPayments(musicoinApi) {
    ...
    checkForPendingReleases(musicoinApi) {
    ...
    updatePendingPlaybackStatus(musicoinApi, r) {
    ...
    updatePendingReleaseStatus(musicoinApi, r) {
...
exports.PendingTxDaemon = PendingTxDaemon;
//# sourceMappingURL=tx-daemon.js.map