...
const musicoin_helper_1 = require("./musicoin-helper");
const FormUtils = require("./form-utils");
const UrlUtils = require("./url-utils");
const MetadataLists = require("../config/metadata-lists");
const json_api_1 = require("./rest-api/json-api");
...
const tx_daemon_1 = require("./tx-daemon");
...
function configure(app, passport, musicoinApi, mediaProvider, config) {
    const serverEndpoint = config.serverEndpoint;
    publicPagesEnabled = config.publicPagesEnabled;
    let mcHelper = new musicoin_helper_1.MusicoinHelper(musicoinApi, mediaProvider, config.playbackLinkTTLMillis);
    const mailSender = new mail_sender_1.MailSender();
    const cachedRequest = new cached_request_1.RequestCache();
    const exchangeRateProvider = new exchange_service_1.ExchangeRateProvider(config.exchangeRateService, cachedRequest);
    let jsonAPI = new json_api_1.MusicoinOrgJsonAPI(musicoinApi, mcHelper, mediaProvider, mailSender, exchangeRateProvider, config);
    let restAPI = new rest_api_1.MusicoinRestAPI(jsonAPI);
    const addressResolver = new address_resolver_1.AddressResolver();
    const releaseManager = new release_manager_routes_1.ReleaseManagerRouter(musicoinApi, jsonAPI, addressResolver, maxImageWidth, mediaProvider, config, doRender);
    const dashboardManager = new admin_dashboard_routes_1.DashboardRouter(musicoinApi, jsonAPI, addressResolver, maxImageWidth, mediaProvider, config, doRender);
    ...
    const newReleaseListener = r => {
        ...
        if (msgText.length > 150)
            msgText = msgText.substring(0, 150) + "...";
    ...
    new tx_daemon_1.PendingTxDaemon(newProfileListener, newReleaseListener)
    ...
    function doRender(req, res, view, context) {
        ...
        const b = req.user && req.user.profileAddress ? musicoinApi.getAccountBalance(req.user.profileAddress) : bluebird_1.Promise.resolve(null);
    ...
    app.get('/main', isLoggedIn, function (req, res) {
        ...
        bluebird_1.Promise.join(rs, fa, b, h, tpw, ttw, function (releases, artists, balance, hero, topPlayed, topTipped) {
    ...
    app.get('/feed', isLoggedIn, function (req, res) {
        ...
        bluebird_1.Promise.join(m, h, tpw, ttw, r, function (messages, hero, topPlayed, topTipped, recentlyPlayed) {
    ...
    function handleBrowseRequest(req, res, _search, genre) {
        ...
        bluebird_1.Promise.join(rs, as, function (releases, artists) {
    ...
    app.get('/tx/history/:address', isLoggedIn, function (req, res) {
        ...
        bluebird_1.Promise.join(musicoinApi.getTransactionHistory(req.params.address, length, start), addressResolver.lookupAddress(req.user.profileAddress, req.params.address), function (history, name) {
    ...
    app.post('/admin/user/abuse', (req, res) => {
        ...
        if (!id)
            return res.json({ success: false, reason: "No id" });
        if (typeof req.body.blacklist == "undefined")
            return res.json({ success: false, reason: "specify true/false for 'blacklist' parameter" });
    ...
    app.get('/admin/licenses/dump', isLoggedIn, adminOnly, function (req, res) {
        ...
        jsonAPI.getAllContracts()
            .then(function (all) {
            return bluebird_1.Promise.all(all.map(contract => {
    ...
    app.get('/admin/overview', isLoggedIn, adminOnly, function (req, res) {
        ...
        bluebird_1.Promise.join(b, o, wp, wr, tc, dtc, au, (mcBalance, balances, usersWithProfile, usersWithRelease, trackCount, deletedTrackCount, allReleaseStats) => {
            const output = [];
    ...
    app.post('/admin/invites/add', (req, res) => {
        if (!req.body.id)
            return res.json({ success: false, reason: "No id" });
        if (!req.body.count)
            return res.json({ success: false, reason: "Invite count to add not provided" });
    ...
    app.post('/admin/users/lock', (req, res) => {
        if (!req.body.id)
            return res.json({ success: false, reason: "No id" });
        if (typeof req.body.lock == "undefined")
            return res.json({ success: false, reason: "specify true/false for 'lock' parameter" });
        User.findById(req.body.id).exec()
            .then(user => {
            user.accountLocked = req.body.lock == "true";
            return user.save();
    ...
    app.get('/artist/:address', isLoggedInOrIsPublic, function (req, res) {
        ...
        bluebird_1.Promise.join(a, m, h, r, x, (output, messages, hero, statsReport, exchangeRate) => {
            if (!output)
    ...
    app.get('/track/:address', isLoggedInOrIsPublic, function (req, res) {
        ...
        bluebird_1.Promise.join(l, ms, r, x, votesPromise, (license, messages, release, exchangeRate, votes) => {
            if (!license || !release) {
                console.log(`Failed to load track page for license: ${address}, err: Not found`);
                return res.render('not-found.ejs');
            }
    ...
    app.get('/profile', isLoggedIn, function (req, res) {
        ...
        bluebird_1.Promise.join(a, r, (output, exchangeRate) => {
            output['invited'] = {
    ...
    app.post('/follows', function (req, res) {
        ...
        if (!req.user.profileAddress)
            return res.json({ success: false, authenticated: true, profile: false });
            ...
            if (result.following) {
                if (req.body.licenseAddress) {
                    Release.findOne({ contractAddress: req.body.licenseAddress }).exec()
                        .then(release => {
                        if (release) {
                            return jsonAPI.postLicenseMessages(req.body.licenseAddress, null, req.user.profileAddress, `${req.user.draftProfile.artistName} is now following ${release.artistName}`, MESSAGE_TYPES.follow, null);
                        }
                        return null;
                    })
                        .catch(err => {
                    ...
                    User.findById(req.body.toFollow).exec()
                        .then((followedUser) => {
                        return jsonAPI.postLicenseMessages(null, followedUser.profileAddress, req.user.profileAddress, `${req.user.draftProfile.artistName} is now following ${followedUser.draftProfile.artistName}`, MESSAGE_TYPES.follow, null);
                    });
                }
            }
        })
    ...
    app.post('/tip', function (req, res) {
        ...
        if (req.user.profileAddress == contractAddress)
            return res.json({ success: false, authenticated: true, profile: true, self: true });
            ...
            return musicoinApi.sendFromProfile(req.user.profileAddress, req.body.recipient, req.body.amount)
                ...
                if (req.body.contextType == "TrackMessage") {
                    ...
                    return jsonAPI.addToReleaseTipCount(req.body.recipient, amount)
                        .then(release => {
                        if (release) {
                            return jsonAPI.postLicenseMessages(req.body.recipient, null, req.user.profileAddress, `${req.user.draftProfile.artistName} tipped ${req.body.amount} ${units} on "${release.title}"`, MESSAGE_TYPES.tip, null);
                        }
                    });
                }
                    ...
                    return jsonAPI.addToUserTipCount(req.body.recipient, amount)
                        .then(tippedUser => {
                        if (tippedUser) {
                            return jsonAPI.postLicenseMessages(null, tippedUser.profileAddress, req.user.profileAddress, `${req.user.draftProfile.artistName} tipped ${req.body.amount} ${units} to ${tippedUser.draftProfile.artistName}!`, MESSAGE_TYPES.tip, null);
                        }
                    });
                }
                    ...
                    return jsonAPI.postLicenseMessages(null, null, req.user.profileAddress, msg, MESSAGE_TYPES.donate, null);
                }
            })
                .catch(function (err) {
                console.log(err);
        ...
        form.parse(req, (err, fields, files) => {
            ...
            const uploadImage = (!files.photo || files.photo.size == 0)
                ? (profile.ipfsImageUrl && profile.ipfsImageUrl.trim().length > 0)
                    ? bluebird_1.Promise.resolve(profile.ipfsImageUrl)
                    : bluebird_1.Promise.resolve(defaultProfileIPFSImage)
            ...
            const uploadHeroImage = (!files.hero || files.hero.size == 0)
                ? (profile.heroImageUrl && profile.heroImageUrl.trim().length > 0)
                    ? bluebird_1.Promise.resolve(profile.heroImageUrl)
            ...
            bluebird_1.Promise.join(uploadImage, uploadHeroImage, (imageUrl, heroImageUrl) => {
                ...
                return bluebird_1.Promise.join(d, s, (descriptionUrl, socialUrl) => {
    ...
    app.post('/license/view/', (req, res) => {
        ...
        jsonAPI.getLicense(req.body.address)
            ...
            return bluebird_1.Promise.join(addressResolver.resolveAddresses(address, license.contributors), function (contributors) {
    ...
    function convertFormToLicense(artistName, selfAddress, fields) {
        ...
        return bluebird_1.Promise.join(addressResolver.resolveAddresses(selfAddress, recipients.contributors), addressResolver.resolveAddresses(selfAddress, recipients.royalties), function (resolvedContributors, resolveRoyalties) {
            const license = {
                coinsPerPlay: 1,
    ...
    function doValidation(license) {
        const errors = [];
        if (!license.royalties.every(r => !r.invalid))
            errors.push("Invalid addresses");
        if (!license.contributors.every(r => !r.invalid))
            errors.push("Invalid addresses");
        if (!(license.title && license.title.trim() != ""))
    ...
    app.post('/license/delete', isLoggedIn, hasProfile, function (req, res) {
        ...
        if (!contractAddress)
            return res.json({ success: false, message: "Not contractAddress was specified" });
    ...
    app.get('/login/reset', redirectIfLoggedIn(loginRedirect), (req, res) => {
        ...
        if (!code)
            return doRender(req, res, "landing.ejs", { message: failMessage });
    ...
    function validateNewAccount(errRedirect) {
        return function (req, res, next) {
            ...
            return bluebird_1.Promise.join(cc, eu, cp, function (confirmation, existingUser, captchaOk) {
                if (!captchaOk) {
    ...
    function getPlaybackEligibility(req) {
        ...
        if (canUseCache)
            return bluebird_1.Promise.resolve({ success: true, canUseCache: true });
        return Release.findOne({ contractAddress: address, state: "published" }).exec()
            ...
            return User.findOne({ profileAddress: release.artistAddress })
                ...
                return bluebird_1.Promise.join(p, b, l, (pendingPayments, profileBalance, license) => {
                    if (payFromProfile) {
    ...
    function payForPPPKey(req, release, license, payFromProfile) {
        const user = req.isAuthenticated() ? req.user : req.anonymousUser;
        ...
        if (payFromProfile) {
            ...
            paymentPromise = musicoinApi.pppFromProfile(user.profileAddress, licenseAddress)
                ...
                return jsonAPI.addPendingPPPTransaction(user._id, release._id, license.coinsPerPlay, keyResponse.transactions.paymentToHotWalletTx)
                    .then(() => {
                    return keyResponse;
                });
            });
    ...
    function getPPPKeyForUser(req, release, license, playbackEligibility) {
        ...
        return playbackEligibility.canUseCache && cachedKey
            ? bluebird_1.Promise.resolve({ key: cachedKey, cached: true })
    ...
    app.get('/ppp/:address', populateAnonymousUser, sendSeekable, resolveExpiringLink, function (req, res) {
        getPlaybackEligibility(req)
            ...
            return bluebird_1.Promise.join(l, r, (license, release) => {
        ...
        if (user) {
            ...
            if (req.user.pendingInitialization) {
                ...
                if (fixFacebook || fixTwitter) {
                    if (fixFacebook)
                        user.facebook.url = `https://www.facebook.com/app_scoped_user_id/${user.facebook.id}/`;
                    if (fixTwitter)
                        user.twitter.url = `https://twitter.com/${user.twitter.username}/`;
                    return user.save()
                        .then(() => {
