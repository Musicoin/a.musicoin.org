...
router.use('/admin', functions.isLoggedIn, functions.adminOnly);
router.use('/admin/*', functions.isLoggedIn, functions.adminOnly);
router.use('/admin/', dashboardManager.getRouter());

router.post('/admin/hero/select', (req, res) => {
...
router.post('/admin/user/abuse', (req, res) => {
...
router.get('/admin/su', functions.isLoggedIn, functions.adminOnly, function (req, res) {
    // render the page and pass in any flash data if it exists
    res.render('su.ejs', { message: req.flash('loginMessage') });
});

router.post('/admin/su', functions.isLoggedIn, functions.adminOnly, passport.authenticate('local-su', {
...
router.get('/admin/licenses/dump', functions.isLoggedIn, functions.adminOnly, function (req, res) {
    // render the page and pass in any flash data if it exists
...
router.get('/admin/artists/dump', functions.isLoggedIn, functions.adminOnly, function (req, res) {
    // render the page and pass in any flash data if it exists
...
router.get('/admin/overview', functions.isLoggedIn, functions.adminOnly, function (req, res) {
    // render the page and pass in any flash data if it exists
    ...
    Promise.join(b, o, wp, wr, tc, dtc, au, (mcBalance, balances, usersWithProfile, usersWithRelease, trackCount, deletedTrackCount, allReleaseStats) => {
        ...
        return functions.doRender(req, res, 'admin-overview.ejs', {
            accounts: output,
...
router.get('/admin/mail/confirm', functions.isLoggedIn, functions.adminOnly, function (req, res) {
    res.render("mail/email-confirmation.ejs", {
        code: "XY12345"
    })
});

router.get('/admin/mail/reset', functions.isLoggedIn, functions.adminOnly, function (req, res) {
    res.render("mail/password-reset.ejs", {
        link: "http://google.com?test=123455"
    })
});

router.get('/admin/mail/invite', functions.isLoggedIn, functions.adminOnly, function (req, res) {
...
router.get('/admin/mail/message', functions.isLoggedIn, functions.adminOnly, function (req, res) {
...
router.get('/admin/errors', (req, res) => {
    ...
    jsonAPI.getErrors(req.query.search, start, length)
        ...
        .then(errors => {
            return functions.doRender(req, res, 'admin-errors.ejs', {
...
router.get('/admin/users', (req, res) => {
    ...
    jsonAPI.getAllUsers(req.query.search, null, null, null, start, length)
        .then(results => {
            ...
            return functions.doRender(req, res, 'admin-users.ejs', {
                search: req.query.search,
                users: users,
                navigation: {
...
router.get('/admin/contacts', (req, res) => {
    ...
    jsonAPI.getAddressBook(req.query.search, start, length)
        .then(users => {
            return functions.doRender(req, res, 'admin-contacts.ejs', {
                search: req.query.search,
                users: users,
                navigation: {
                    show10: `${url}&length=10`,
...
router.get('/admin/releases', (req, res) => {
    ...
    jsonAPI.getAllReleases(req.query.search, start, length)
        .then(result => {
            const releases = result.releases;
            return functions.doRender(req, res, 'admin-releases.ejs', {
                search: req.query.search,
                releases: releases,
                navigation: {
                    show10: `${url}&length=10`,
...
router.get('/peerverif/a7565fbd8b81b42031fd893db7645856f9d6f377a188e95423702e804c7b64b1', (req, res) => {
    ...
    jsonAPI.getAllUsers(req.query.search, null, null, null, start, length)
        .then(results => {
            const users = results.users;
            return functions.doRender(req, res, 'peer-verification.ejs', {
                search: req.query.search,
                users: users
            });
        });
...
router.get('/playback-history/a6565fbd8b81b42031fd893db7645856f9d6f377a188e95423702e804c7b64b1', functions.isLoggedIn, functions.adminOnly, function (req, res) {
    const length = 1000;
    const start = 0;
    ...
    jsonAPI.getPlaybackHistory(req.body.user, req.body.anonuser, req.body.release, start, length)
        ...
        .then(output => {
            functions.doRender(req, res, 'playback-main.ejs', {
