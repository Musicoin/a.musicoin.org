...
class ReleaseManagerRouter {
    constructor(musicoinApi, jsonAPI, addressResolver, maxImageWidth, mediaProvider, // TODO
        config, doRender) {
        ...
        router.post('/edit', function (req, res) {
            jsonAPI.getLicense(FormUtils.requiredString(req.body.contractAddress))
                .then(license => {
                if (!license)
                    throw new Error(`Could not find license to update: ${req.body.contractAddress}`);
        ...
        router.post('/update', function (req, res) {
            ...
            form.parse(req, (err, fields, files) => {
                ...
                jsonAPI.getLicense(fields.contractAddress)
                    .then(license => {
                    if (!license)
                        throw new Error(`Could not find license to update: ${fields.contractAddress}`);
                    if (req.user.profileAddress != license.artistProfileAddress)
                    ...
                    if (!fields.title)
                        return res.json({ success: false, reason: "You must provide a title" });
                    ...
                    return bluebird_1.Promise.join(i, m, c, function (imageUrl, metadataUrl, contributors) {
                        track.imageUrl = imageUrl;
                        return musicoinApi.updateTrack(fields.contractAddress, track.title, imageUrl, metadataUrl, contributors).then(txs => {
        ...
        router.get('/resolve/:address', function (req, res) {
        ...
        router.post('/release', function (req, res) {
            ...
            form.parse(req, (err, fields, files) => {
                console.log(`Fields: ${JSON.stringify(fields)}`);
                console.log(`Files: ${JSON.stringify(files)}`);
                if (!isVerified(req.user) && !hasSocialLinks(req.user))
                    return res.json({ success: false, reason: "You must link a twitter or facebook account to release music" });
                if (!fields.title)
                    return res.json({ success: false, reason: "You must provide a title" });
                if (!files.audio || files.audio.size == 0)
                    return res.json({ success: false, reason: "You must provide an audio file" });
                if (fields.rights != "confirmed")
                    return res.json({ success: false, reason: "You must confirm that you have rights to release this work." });
                if (config.termsOfUseVersion != req.user.termsOfUseVersion && fields.terms != "confirmed")
                    return res.json({ success: false, reason: "You must agree to the Musicoin website's Terms of Use before you can release music." });
                const track = buildTrackFromForm(req, fields, files);
                const selfAddress = req.user.profileAddress;
                ...
                return bluebird_1.Promise.join(a, i, m, c, r, function (audioUrl, imageUrl, metadataUrl, contributors, royalties) {
                    track.imageUrl = imageUrl;
                    return musicoinApi.releaseTrack(req.user.profileAddress, req.user.draftProfile.artistName, track.title, imageUrl, metadataUrl, audioUrl, contributors, royalties, track.audio.type, key);
                })
                    .then(function (tx) {
        ...
        function buildTrackFromForm(req, fields, files) {
...
exports.ReleaseManagerRouter = ReleaseManagerRouter;
//# sourceMappingURL=release-manager-routes.js.map