      ...
      if (err == null) {
        ...
        fs.stat(config.streaming.org + '/' + req.params.address + '/' + req.params.address + '.mp3', function (err) {
          if (err == null) {
            ...
            require('child_process').exec('ffmpeg -re -i ' + config.streaming.org + '/' + req.params.address + '/' + req.params.address + '.mp3' + ' -codec copy -bsf h264_mp4toannexb -map 0 -f segment -segment_time ' + config.streaming.segments + ' -segment_format mpegts -segment_list ' + config.streaming.org + '/' + req.params.address + '/' + 'index.m3u8 -segment_list_type m3u8 ' + config.streaming.org + '/' + req.params.address + '/ts%d.ts && mv ' + config.streaming.org + '/' + req.params.address + '/*.m3u8 *.ts ' + '&& cd ' + config.streaming.tracks + '/' + ' && mkdir ' + req.params.address + ' && cd ' + config.streaming.org + '/' + req.params.address + '/&& find . ' + "-regex '.*\.\(ts\|m3u8\)' -exec mv {} " + config.streaming.tracks + '/' + req.params.address + '/');
          } else if (err.code == 'ENOENT') {
            aria2.open();
